<div>
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mt-8 mb-4 px-6">
        <div>
            <h1 class="text-2xl font-bold text-primary">Gestión de soportes de los negocios</h1>
            <p class="text-sm text-ink mt-1">Administra los soportes de las facturas del negocio</p>
        </div>
        <a href="{{ route('admin.businessReports.create') }}"
        ...
        <div class="p-5 border-b border-neutral-300 bg-white">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div class="flex items-center">
                    ...
                    <h2 class="text-lg font-semibold text-ink">Lista de soportes de negocios</h2>
                </div>
                <!-- Buscador mejorado -->
                <div class="relative">
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-neutral-50">
                    <tr class="text-xs font-medium text-primary uppercase tracking-wider">
                        ...
                        <th scope="col" class="px-6 py-4 text-left">Id negocio</th>
                        <th scope="col" class="px-6 py-4 text-left">User Dni</th>
                        <th scope="col" class="px-6 py-4 text-left">Total</th>
                        <th scope="col" class="px-6 py-4 text-left">Comisión</th>
                    ...
                    @forelse ($businessReports as $report)
                        <div wire:key="{{ $report->id }}">
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                <td class="px-6 py-4">{{ $report->invoice_date }}</td>
                                <th scope="row" class="px-6 py-4">
                                    {{ $report->id }}
                                </th>
                                <td class="px-6 py-4">
                                    <flux:tooltip content="" position="top">
                                        <button type="button">{{ $report->business_data_id }}</button>
                                    </flux:tooltip>
                                </td>
                                <td class="px-6 py-4">{{ $report->user_dni }}</td>
                                <td class="px-6 py-4">{{ $report->total_amount }}</td>
                                <td class="px-6 py-4">{{ $report->commission }}</td>
                                <td class="px-6 py-4">{{ $report->status }}</td>
                                <td class="px-6 py-4">{{ $report->invoice_number }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-right">
                                    <div class="flex items-center justify-end space-x-3">

                                        @if ($report->status === 'pending_review')
                                            <flux:button wire:click="compare({{ $report->id }})" class="mr-4"
                                                size="xs" icon="hand-thumb-up" />
                                        @endif

                                        {{-- Editar --}}
                                        <a href="{{ route('admin.businessReports.edit', $report) }}"
                                            class="text-secondary hover:text-secondary/80 transition-colors p-1.5 rounded-md hover:bg-secondary/10">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        {{-- eliminar --}}
                                        <button wire:click="destroy({{ $report->id }})"
                                            wire:confirm="¿Estás seguro de eliminar el reporte {{ $report->invoice_number }}?"
                        ...
                        <tr>
                            <td colspan="4" class="px-6 py-16 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="text-primary font-medium mb-1">No se encontraron reportos</p>
                                    <p class="text-ink text-sm">No hay reportos disponibles para mostrar</p>
    ...
    <flux:modal name="edit-profile" class="md:w-96">
        <div class="space-y-6">
            <div>
                <flux:heading size="lg">Comprobar </flux:heading>
                <flux:text class="mt-2">Comprobar que sean iguales.</flux:text>
            </div>

            @if ($this->total)
                <p> El total de la factura es igual</p>
            @endif

            @if ($this->user)
                <p> Coincide el usuario</p>
            @else
                <p> No coinciden los usuario</p>
            @endif
            ...
            <div class="flex">
                ...
                <flux:button wire:click="approve" type="button" variant="primary">Aprobar y Crear Puntos
                </flux:button>
            </div>
        </div>
    </flux:modal>
